...
// ----------------------------------------------------------------
// GET /api/defillama?type=overview|protocol|chain[&name=<slug>]
//
...
async function handleProtocol(name: string): Promise<NextResponse<ProtocolDetailResponse>> {
  const data = (await fetchJSON(
    `https://api.llama.fi/summary/fees/${encodeURIComponent(name)}`,
  )) as Record<string, unknown>;
...
async function handleChain(name: string): Promise<NextResponse<ChainResponse>> {
  const data = (await fetchJSON(
    `https://api.llama.fi/overview/fees/${encodeURIComponent(name)}`,
  )) as Record<string, unknown>;
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    switch (type) {
      ...
      default:
        return NextResponse.json(
          { error: `Invalid type "${type}". Must be one of: overview, protocol, chain` },
          { status: 400 },
        );
    }
  } catch (err) {
